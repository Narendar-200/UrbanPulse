...
export const Navigation: React.FC<NavigationProps> = ({ mobileNavOpen, onCloseMobileNav }) => {
  ...
  return (
    <>
      <div
        className={`fixed inset-0 bg-slate-900/55 backdrop-blur-[1px] z-40 lg:hidden transition-opacity ${
          mobileNavOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
        }`}
        onClick={onCloseMobileNav}
      />
      <nav
        className={`w-64 bg-gradient-to-b from-slate-900 to-slate-800 border-r border-slate-700/50 h-screen flex flex-col p-5 fixed left-0 top-0 overflow-y-auto z-50 transition-transform duration-300 ${
